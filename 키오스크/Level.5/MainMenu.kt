...
suspend fun main(){
    ...
    while(true){
        ...
        var categorypick=getPureNumber()
        if(categorypick==0){
        ...
        var pickedFood=pickMenu(categorypick)
        globalDelay(3000)
        pickedFood?.let { food ->
            addOrder(food)
        }?: run{
...
fun displayMenu(){
    ...
    var maxNameLength= menus.maxOfOrNull { it.name.length } ?: 0
...
fun pickMenu(cateNumber: Int): Food?{
    var menu =menus[cateNumber-1]
    var categoyName = menu.name

    if(categoyName!="Order"&& categoyName!="Cacel"){
        var fillteredFood = foods.filter { it.category == categoyName}
        displayMenuDetail(categoyName)

        while (true){
            val pickFoodNumber= getPureNumber()
            if(pickFoodNumber>fillteredFood.size||pickFoodNumber<0){
                println("올바른 숫자를 입력해주세요")
            }
            else if(pickFoodNumber==0){
                return null
            }
            else{
                return fillteredFood[pickFoodNumber-1]
            }
        }
        ...
        when(categoyName){
            "Order"->{
                ...
                println("1. 주문\t\t 2. 메뉴판")
                while (true){
                    val pickOrderNumber= getPureNumber()
                    when(pickOrderNumber){
                        1->{
...
fun displayMenuDetail(categoryName: String){
    ...
    var maxNameLength= filteredFoods.maxOfOrNull { it.name.toString().length } ?: 0
...
fun addOrder(food:Food){
    ...
    println("1. 확인\t\t 2. 취소")
    while (true){
        var pickOrderNummber= getPureNumber()
        when(pickOrderNummber){
            1->{
